...
void SSD1306_send_cmd(uint8_t cmd) {
    ...
    i2c_write_blocking(i2c1, SSD1306_ADDR, buf, 2, false);
}
...
void SSD1306_send_buf(uint8_t buf[], int buflen) {
    ...
    i2c_write_blocking(i2c1, SSD1306_ADDR, temp_buf, buflen + 1, false);

    free(temp_buf);
}

void SSD1306_init() {
    // Some of these commands are not strictly necessary as the BITDOG_RESET
    ...
    uint8_t cmds[] = {
        ...
        SSD1306_DISP_HEIGHT - 1,             // Display height - 1
        SSD1306_SET_COM_OUT_DIR | 0x08, // set COM (common) output scan direction. Scan from bottom up, COM[N-1] to COM0
        SSD1306_SET_DISP_OFFSET,        // set display offset
        0x00,                           // no offset
        SSD1306_SET_COM_PIN_CFG,        // set COM (common) pins hardware configuration. Board specific magic number.
                                        // 0x02 Works for 128x32, 0x12 Possibly works for 128x64. Other options 0x22, 0x32
#if ((SSD1306_DISP_WIDTH == 128) && (SSD1306_DISP_HEIGHT == 32))
        0x02,
#elif ((SSD1306_DISP_WIDTH == 128) && (SSD1306_DISP_HEIGHT == 64))
        0x12,
#else
        0x02,
#endif
...
void SetPixel(uint8_t *buf, int x,int y, bool on) {
    assert(x >= 0 && x < SSD1306_DISP_WIDTH && y >=0 && y < SSD1306_DISP_HEIGHT);
    ...
    const int BytesPerRow = SSD1306_DISP_WIDTH ; // x pixels, 1bpp, but each row is 8 pixel high, so (x / 8) * 8
...
void WriteChar(uint8_t *buf, int16_t x, int16_t y, uint8_t ch) {
    if (x > SSD1306_DISP_WIDTH - 8 || y > SSD1306_DISP_HEIGHT - 8)
        return;
